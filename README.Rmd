...
##USArrests
```{r USArrests}
USArrests_tidy <- USArrests %>% mutate(State = rownames(.)) %>% tbl_df
...
```{r USJudgeRatings}
USJudgeRatings_tidy <- USJudgeRatings %>% mutate ( NAME = rownames(.)) %>% tbl_df
...
USPersonalExpenditure_tidy <- USPersonalExpenditure %>% as.data.frame() %>% mutate (Expense = rownames(.)) %>% tbl_df 
...
## WorldPhones
```{r WorldPhones} 
WorldPhones_tidy <- WorldPhones %>% as.data.frame() %>% mutate (year = rownames(.)) %>% gather (continent, value, -year) %>% tbl_df()
